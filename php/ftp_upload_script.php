...
# FTP SERVER SETTINGS
$savedfiledirectory="$uid/"; #hardcode $uid here
include 'ftp.php';
$con = ftp_connection($domainhost, $domainuser, $domainpass, $uid);
ftp_connection_quit($con);
#exit;
    ...
    if (!empty($_POST['country'])) {
        if (!empty($_POST['city'])) {
            if (!empty($_POST['caption'])) {
                ...
                $imagespath = $publiclink.$savedfiledirectory.$userpic;
                if(PutFilesonFTPServer($domainhost, $domainuser, $domainpass, $savedfiledirectory, $userpic, $tmp_name)){ 
                    ...
                    $userID = $userRow[0];
                    $sql = "INSERT INTO `images` VALUE(NULL, '$imagespath', '$caption', '$userID', '$cityID', '$countryID')";
